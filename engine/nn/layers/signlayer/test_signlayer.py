...
class TestSignLayer_constructor(unittest.TestCase):
    ...
    def test_rmi_star_exact(self):
        ...
        test_V = read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_V_ID])
        test_C = np.reshape(read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_C_ID]), (1, 784))
        test_d = np.array([read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_D_ID])])
        test_predicate_lb = read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_PREDICATE_LB_ID])
        test_predicate_ub = read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_PREDICATE_UB_ID])
        
        test_zono_V = read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_ZONO_V_ID])
        test_zono_c = read_csv_data(sources[SIGNL_TEST_RMI_INIT][SIGNL_TEST_RMI_STAR_ZONO_C_ID])
        ...
        result = sign_layer.reach_star_multiple_inputs([test_star1,test_star2], 'approx-star')

    def test_rsi_star_exact(self):
        test_name = 'sign_1'
        test_mode = 'polar_zero_to_pos_one'
        test_num_inputs = 1
        test_input_names = ['in']
        ...
        test_V = read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_V_ID])
        test_C = np.reshape(read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_C_ID]), (1, 784))
        test_d = np.array([read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_D_ID])])
        test_predicate_lb = read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_PREDICATE_LB_ID])
        test_predicate_ub = read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_PREDICATE_UB_ID])
        
        test_zono_V = read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_ZONO_V_ID])
        test_zono_c = read_csv_data(sources[SIGNL_TEST_RSI_INIT][SIGNL_TEST_RSI_STAR_ZONO_C_ID])
    
        test_star = Star(
                test_V, test_C, test_d, test_predicate_lb, test_predicate_ub
            )
        test_star.Z = Zono(test_zono_c, test_zono_V)
